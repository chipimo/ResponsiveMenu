...
class LoginV1 extends React.Component {
      ...
      if (type === "success") {
        ...
        await fetch(
          ...
          .then(json => {
            ...
            this.props.SocketConfig.socket.on("USER_NOT_EXIST", callback => {
              this.setState({
                isLoading: false
              });
              this.closeModal();
              ...
              const toHome = StackActions.reset({
                index: 0,
                actions: [
                  NavigationActions.navigate({ routeName: "FacebookLogin" })
                ]
              });
              this.props.navigation.dispatch(toHome);
            });
            this.props.SocketConfig.socket.on(
              "USER_IS_REGSTARTED",
              callback => {
                if (callback) {
  ...
  renderImage = () => {
    const imageSize = {
      ...
      height: screenSize.height - scaleModerate(400, 1)
    };
    return (
      <Image
  ...
  onLoginButtonPressed = () => {
    ...
    if (!this.state.userTemp.email) {
      this.setState({
        emailErrorMsg: "Email is requred",
        errors: {
          ...this.state.errors,
          email: false
        }
      ...
      Keyboard.dismiss();
      this.openModal();
      this.props.SocketConfig.socket.emit("VERIFY_USER", this.state.userTemp);
      this.props.SocketConfig.socket.on("USER_IS_VERIFYED", callback => {
        this.props.dispatchEvent({
          type: "LOGGEDIN",
          payload: callback.credentials[0]
        });
        this.closeModal();
        const toHome = StackActions.reset({
          index: 0,
      ...
      this.props.SocketConfig.socket.on("USER_NOT_VERIFYED", callback => {
        this.closeModal();
        this.setState({
      ...
      >
        ...
        >
          ...
          {this.state.errors.email ? null : (
            <RkText rkType="danger primary3">{this.state.emailErrorMsg}</RkText>
          )}
        </View>
        ...
        <View style={{ flex: 1, width: "100%", padding: 10 }}>
          <View style={{ marginBottom: 6, paddingTop: 10 }}>
